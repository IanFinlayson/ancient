...
/* converts row,col notation to single dim notation */
int pos( int row, int col )
{
	return col*4 + row;
}

/* multiplies two matrices and puts the answer in result */
void multMatrix( GLfloat* lhs, GLfloat* rhs, GLfloat* result )
{
	for( int row=0; row<4; row++ )
	{
		for( int col=0; col<4; col++ )
		{
			result[pos(row, col)] = 0.0;
			for( int cell=0; cell<4; cell++ )
			{
				result[pos(row, col)] += lhs[pos(row,cell)] * rhs[pos(cell, col)];
			}
		}
	}
}

/* puts the identity matrix into target */
void getIdentity( GLfloat* target )
{
	for( int row=0; row<4; row++ )
	{
		for( int col=0; col<4; col++ )
		{
			target[pos(row, col)] = (row == col) ? 1 : 0;
		}
	}
}

/* Transformations: They all work by getting the current transformation matrix from
   OpenGL, making the appropriate matrix to multiply it by, and give this result to
   OpenGL.  Using a global matrix did not work well with push/pop Matrix */

/* Loads the identity matrix as the current transformation matrix */
...
/* Performs translations just like glTranslatef */
void myTranslatef( GLfloat x, GLfloat y, GLfloat z )
{
	GLfloat trans[16];		/* Our translation matrix */
	getIdentity( trans );
	trans[pos(0, 3)] = x;
	trans[pos(1, 3)] = y;
	trans[pos(2, 3)] = z;

	GLfloat current[16];	/* The current transformation matrix */
	glGetFloatv( GL_MODELVIEW_MATRIX, current );

	GLfloat result[16];		/* multiply them */
	multMatrix( current, trans, result );

	glLoadMatrixf( result );	/* set this as the new matrix */
}

/* Performs rotations around the X axis */
void myRotatefX( GLfloat angle )
{
	GLfloat rot[16];		/* Our rotation matrix */
	getIdentity( rot );
	rot[pos(1,1)] = cos( angle*PI / 180.0 );
	rot[pos(2,2)] = cos( angle*PI / 180.0 );
	rot[pos(1,2)] = -sin( angle*PI / 180.0 );
	rot[pos(2,1)] = sin( angle*PI / 180.0 );

	GLfloat current[16];	/* The current transformation matrix */
	glGetFloatv( GL_MODELVIEW_MATRIX, current );

	GLfloat result[16];		/* multiply them */
	multMatrix( current, rot, result );

	glLoadMatrixf( result );	/* set this as the new matrix */
}

/* Perform rotations around the Y axis */
void myRotatefY( GLfloat angle )
{
	GLfloat rot[16];		/* Our rotation matrix */
	getIdentity( rot );
	rot[pos(0, 0)] = cos( angle*PI / 180.0 );
	rot[pos(2, 2)] = cos( angle*PI / 180.0 );
	rot[pos(2, 0)] = -sin( angle*PI / 180.0 );
	rot[pos(0, 2)] = sin( angle*PI / 180.0 );
...
/* Perform rotations around the Z axis */
void myRotatefZ( GLfloat angle )
{
	GLfloat rot[16];		/* Our rotation matrix */
	getIdentity( rot );
	rot[pos(0, 0)] = cos( angle*PI / 180.0 );
	rot[pos(1, 1)] = cos( angle*PI / 180.0 );
	rot[pos(0, 1)] = -sin( angle*PI / 180.0 );
	rot[pos(1, 0)] = sin( angle*PI / 180.0 );
...
/* Performs rotations just like glRotatef (except it only allows one axis
   at a time to be rotated on). I made this function so I could more easily search/replace the old code */
void myRotatef( GLfloat angle, GLfloat x, GLfloat y, GLfloat z )
{
	/* Chooses axis based on which x/y/x is greatest */
...
/* Scales the current matrix just like glScalef does */
void myScalef( GLfloat x, GLfloat y, GLfloat z )
{
	GLfloat scale[16];		/* Our translation matrix */
	getIdentity( scale );
	scale[pos(0, 0)] = x;
	scale[pos(1, 1)] = y;
	scale[pos(2, 2)] = z;

	GLfloat current[16];	/* The current trans matrix */
	glGetFloatv( GL_MODELVIEW_MATRIX, current );

	GLfloat result[16];		/* multiply them */
	multMatrix( current, scale, result );
...
void drawLeftWing( void )
{
	...
	glPushMatrix( );
	myTranslatef( up_left_x, (up_left_y + down_left_y) / 2.0, (up_left_z + up_right_z) / 2.0 );
	myRotatef( wing_flap_amount, 0.0, 0.0, 1.0 );
	myTranslatef( -up_left_x, (up_left_y + down_left_y) / -2.0, (up_left_z + up_right_z) / -2.0 );
...
void drawLeftFoot( void )
{
	...
	myTranslatef( (front_left_x + front_right_x) /2.0, front_left_y, (front_left_z + front_right_z) / 2.0  );
	myRotatef( foot_move_amount, 1.0, 0.0, 0.0 );
	myTranslatef( (front_left_x + front_right_x) /-2.0, -front_left_y, (front_left_z + front_right_z) / -2.0  );
...
void drawPenguin( void )
{
	...
	glPushMatrix( );
		...
		myRotatef( wobble_amount, 0.0, 0.0, 1.0 );
		drawHead( );
		drawBody( );
	...
	foot_move_amount = myabs( 90.0 - foot_move_amount );
	drawLeftFoot( );
...
void display( void )
{
	...
	/* move him */
	myTranslatef( penguin_x, penguin_y, 0.0 );

	/* rotate it */
	myRotatef( rot_amount, 0.0, 1.0, 0.0 );

	/* zoom in/out */
	myScalef( scale_amount, scale_amount, scale_amount );
...
void animate( void )
{
	...
	if( walking )
	{
		...
		if( myabs( desired_x - penguin_x ) < 8.0 )
		{
			if( myabs( desired_y - penguin_y ) < 8.0 )
			{
				walking = 0;
			}
		}
